	...
	React.useEffect(() => {
		try {
			if (!G.inited && !G.loading) {
				...
				fetch(getApiUrl('all-tokens'), { method: 'POST', headers: { 'Content-Type': 'application/json' } }).then(data => data.json()).then(coins => {
					if (coins) {
		...
		try {
			if (G.status === CONNECTED) {
				...
				if (token && amount > 0) {
					...
					const rbalance1 = G.targetChain === 'ICICB'|| G.token === "ICICB"? value : await G.bridgebalance(G.targetChain, targetToken.address)
